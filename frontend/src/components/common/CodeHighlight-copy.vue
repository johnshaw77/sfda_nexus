...
const renderMarkdown = (content) => {
  ...
  try {
    ...
    renderer.code = function (code, lang, escaped) {
      ...
      return `${actualLang}12333<pre role="region" aria-label="代碼塊"><code class="${langClass}" aria-label="${actualLang || "代碼"}">${escapeHtml(codeContent)}</code></pre>`;
    };

    // 配置 marked
    marked.setOptions({
      breaks: true,
      gfm: true, // 是否啟用 GitHub Flavored Markdown
      tables: true, // 是否啟用表格
      smartLists: true, // 是否啟用智能列表
      smartypants: true, // 是否啟用智能引號
      // 自定義渲染器
      renderer: renderer,
      // highlight: function (code, lang) {
      //   console.log(lang, "lan09009g");
        ...
        if (language && Prism.languages[language]) {
          try {
            ...
            if (code && code.trim()) {
              ...
              //todo
              console.log(`CodeHighlight12333: 程式碼高亮完成 (${language})`);
            }
          } catch (error) {
