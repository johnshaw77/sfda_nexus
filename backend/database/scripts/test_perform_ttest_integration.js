...
async function runIntegrationTest() {
  ...
  try {
    ...
    if (toolCallResponse.data.success) {
      ...
      // 處理嵌套的結果結構
      let statResult = null;
      if (result.data && result.data.result && result.data.result.data && result.data.result.data.result) {
        statResult = result.data.result.data.result;
      } else if (result.data && result.data.result) {
        statResult = result.data.result;
      }
