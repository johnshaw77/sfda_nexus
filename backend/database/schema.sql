...
CREATE TABLE users (
    ...
    avatar TEXT DEFAULT NULL COMMENT '頭像',
...
CREATE TABLE agents (
    ...
    avatar TEXT DEFAULT NULL COMMENT '智能體頭像',
    ...
    agent_type ENUM('custom', 'qwen') DEFAULT 'custom' COMMENT 'Agent 類型',
    tags JSON DEFAULT NULL COMMENT '標籤',
    capabilities JSON DEFAULT NULL COMMENT '能力配置',
    tools JSON DEFAULT NULL COMMENT '可用工具配置',
    qwen_config JSON DEFAULT NULL COMMENT 'Qwen-Agent 專用配置',
    tool_selection_mode ENUM('manual', 'auto') DEFAULT 'manual' COMMENT '工具選擇模式',
    is_active BOOLEAN DEFAULT TRUE COMMENT '是否啟用',
    ...
    INDEX idx_tool_selection_mode (tool_selection_mode),
    INDEX idx_active (is_active),
