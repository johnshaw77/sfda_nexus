...
import { catchAsync, createSuccessResponse, ValidationError, AuthenticationError, BusinessError } from '../middleware/errorHandler.middleware.js';
...
const schemas = {
  ...
  register: Joi.object({
    ...
    phone: Joi.string().pattern(/^[\d\-\+\(\)]{10,20}$/).optional().messages({
...
export const handleUpdateProfile = catchAsync(async (req, res) => {
  ...
  // 過濾不允許更新的字段
  const allowedFields = ['display_name', 'avatar_url', 'department', 'position', 'phone', 'preferences'];
  const filteredData = {};
...
export const handleChangePassword = catchAsync(async (req, res) => {
  ...
  // 驗證當前密碼
  const isCurrentPasswordValid = await UserModel.verifyPassword(current_password, rows[0].password_hash);
  if (!isCurrentPasswordValid) {
    throw new AuthenticationError('當前密碼錯誤');
  }

  // 檢查新密碼是否與當前密碼相同
  const isSamePassword = await UserModel.verifyPassword(new_password, rows[0].password_hash);
  if (isSamePassword) {
