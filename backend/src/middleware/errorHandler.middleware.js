...
export const errorHandler = (error, req, res, next) => {
  ...
  if (!error.isOperational) {
    ...
    // 一般錯誤
      ...
      // 保留原始堆棧信息
      processedError.originalStack = originalStack;
      processedError.originalError = originalError;
    }
  }
  ...
  // 在開發環境下，將原始錯誤信息添加到響應中
  if (appConfig.server.env === "development" && processedError.originalStack) {
    processedError.stack = processedError.originalStack;
  }

  // 發送錯誤響應
  res
    .status(processedError.statusCode || 500)
    .json(formatErrorResponse(processedError, req));
};
